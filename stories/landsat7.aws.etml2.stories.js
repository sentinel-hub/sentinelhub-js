...
if (!process.env.LANDSAT7_LETML2_LAYER_ID) {
  throw new Error('LANDSAT7_LETML2_LAYER_ID environment variable is not defined!');
}

if (!process.env.LANDSAT7_LETML2_NDVI_LAYER_ID) {
  throw new Error('LANDSAT7_LETML2_NDVI_LAYER_ID environment variable is not defined!');
}

const instanceId = process.env.INSTANCE_ID;
const layerId = process.env.LANDSAT7_LETML2_LAYER_ID;
const layerIdNdvi = process.env.LANDSAT7_LETML2_NDVI_LAYER_ID;
...
const fromTime = new Date(Date.UTC(1999, 7 - 1, 22, 0, 0, 0));
const toTime = new Date(Date.UTC(2000, 8 - 1, 22, 23, 59, 59));
...
export const getMapURL = () => {
  ...
  const layer = new Landsat7AWSLETML2Layer({ instanceId, layerId });
  ...
  const perform = async () => {
    const layer = (
      await LayersFactory.makeLayers(
        `${DATASET_AWS_LETML2.shServiceHostname}ogc/wms/${instanceId}`,
...
export const getMapWMSEvalscript = () => {
  ...
  const perform = async () => {
    const layer = new Landsat7AWSLETML2Layer({
      instanceId,
      layerId,
      evalscript: `
...
export const findDatesUTCSearchIndex = () =>
  createFindDatesUTCStory(
    new Landsat7AWSLETML2Layer({ instanceId, layerId, maxCloudCoverPercent: 40 }),
    bbox4326,
    fromTime,
    toTime,
    false,
...
export const stats = () => {
  ...
  const layer = new Landsat7AWSLETML2Layer({ instanceId, layerId: layerIdNdvi, maxCloudCoverPercent: 100 });
