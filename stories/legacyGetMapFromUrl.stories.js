...
export const ProcessingLegacyGetMapFromUrlWithGain = () => {
  if (!process.env.CLIENT_ID || !process.env.CLIENT_SECRET) {
    return "<div>Please set OAuth Client's id and secret for Processing API (CLIENT_ID, CLIENT_SECRET env vars)</div>";
  }
  ...
  const wrapperEl = document.createElement('div');
  wrapperEl.innerHTML = `<h2>Processing LegacyGetMapFromUrl With WMS Fallback; no gain vs gain=${gain}</h2>`;
  ...
  const perform = async () => {
    ...
    try {
      const imageBlobGainIs1 = await legacyGetMapFromUrl(
        `${baseUrl}?${queryParams}`,
        ApiType.PROCESSING,
        true,
      );
...
export const ProcessingLegacyGetMapFromUrlWithGamma = () => {
  if (!process.env.CLIENT_ID || !process.env.CLIENT_SECRET) {
    return "<div>Please set OAuth Client's id and secret for Processing API (CLIENT_ID, CLIENT_SECRET env vars)</div>";
  }
  ...
  const wrapperEl = document.createElement('div');
  wrapperEl.innerHTML = `<h2>Processing LegacyGetMapFromUrl With WMS Fallback; no gamma vs gamma=${gamma}</h2>`;
  ...
  const perform = async () => {
    ...
    try {
      const imageBlobGammaIs1 = await legacyGetMapFromUrl(
        `${baseUrl}?${queryParams}`,
        ApiType.PROCESSING,
        true,
      );
...
export const ProcessingLegacyGetMapFromUrlWithGainAndGamma = () => {
  if (!process.env.CLIENT_ID || !process.env.CLIENT_SECRET) {
    return "<div>Please set OAuth Client's id and secret for Processing API (CLIENT_ID, CLIENT_SECRET env vars)</div>";
  }
  ...
  const wrapperEl = document.createElement('div');
  wrapperEl.innerHTML = `<h2>Processing LegacyGetMapFromUrl With WMS Fallback; no gain/gamma vs gain=${gain} and gamma=${gamma}</h2>`;
...
export const WMSLegacyGetMapFromUrlWithGain = () => {
  const imgGainIs1 = document.createElement('img');
  imgGainIs1.width = '512';
  imgGainIs1.height = '512';
  ...
  const wrapperEl = document.createElement('div');
  wrapperEl.innerHTML = `<h2>WMS LegacyGetMapFromUrl; no gain vs gain=${gain}</h2>`;
  wrapperEl.insertAdjacentElement('beforeend', imgGainIs1);
  wrapperEl.insertAdjacentElement('beforeend', imgGainIs2);

  const perform = async () => {
    try {
      const imageBlobGainIs1 = await legacyGetMapFromUrl(`${baseUrl}?${queryParams}`);
      imgGainIs1.src = URL.createObjectURL(imageBlobGainIs1);
...
export const WMSLegacyGetMapFromUrlWithGamma = () => {
  const imgGammaIs1 = document.createElement('img');
  imgGammaIs1.width = '512';
  imgGammaIs1.height = '512';
  ...
  const wrapperEl = document.createElement('div');
  wrapperEl.innerHTML = `<h2>WMS LegacyGetMapFromUrl; no gamma vs gamma=${gamma}</h2>`;
  wrapperEl.insertAdjacentElement('beforeend', imgGammaIs1);
  wrapperEl.insertAdjacentElement('beforeend', imgGammaIs2);

  const perform = async () => {
    try {
      const imageBlobGammaIs1 = await legacyGetMapFromUrl(`${baseUrl}?${queryParams}`);
      imgGammaIs1.src = URL.createObjectURL(imageBlobGammaIs1);
...
export const WMSLegacyGetMapFromUrlWithGainAndGamma = () => {
  const imgNoGainGamma = document.createElement('img');
  imgNoGainGamma.width = '512';
  imgNoGainGamma.height = '512';
  ...
  const wrapperEl = document.createElement('div');
  wrapperEl.innerHTML = `<h2>WMS LegacyGetMapFromUrl; no gain/gamma vs gain=${gain} and gamma=${gamma}</h2>`;
  wrapperEl.insertAdjacentElement('beforeend', imgNoGainGamma);
  wrapperEl.insertAdjacentElement('beforeend', imgGainGammaAre2);
